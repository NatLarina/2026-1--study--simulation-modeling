...
![Создание нового релиза v.4.0.0](1s.png){#fig-release width=70%}

### 2. Настройка окружения Julia

Запускаем Julia и установливаем пакеты

![Установка пакетов Julia](2s.png){#fig-julia-start width=70%}

### 3. Запуск скриптов и тема работы

Создаем файлы и запускаем их

![Установка пакетов Julia](3s.png){#fig-julia-start width=70%}

### 4. Графики

## 1. График динамики S(I)R (рис. 4.1)

![](sir_basic_dynamics.png){#fig-julia-start width=70%}
...
- **Конечная доля переболевших** – асимптота R(t) (или S(t) → 0).  
  - *Вывод:* сколько всего людей переболело (или приобрело иммунитет). Сравнить с теоретическим значением 1 – 1/R₀.
...
![](beta_scan.png){#fig-julia-start width=70%}
...
- **Порог возникновения эпидемии** – значение β, при котором пик начинает превышать 0 (или небольшой порог, например 1% популяции).  
  ...
  - *Сравнение с теорией:* порог соответствует R₀ = β/γ = 1, т.е. β = γ = 1/infection_period.
...
> «Пороговое значение β ≈ 0.15, что соответствует R₀ ≈ 2.1 (γ = 1/14). При β = 0.3 пик достигает 40%, а смертность – 1.5% от популяции. Рост смертности нелинейный: удвоение β увеличивает смертность в 3 раза.»

---

## 3. График эффекта миграции (рис. 4.3)

![](migration_effect.png){#fig-julia-start width=70%}
...
![](comprehensive_analysis.png){#fig-julia-start width=70%}
